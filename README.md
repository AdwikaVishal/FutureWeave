# ⟁ Decision Simulator

A multi-timeline life simulation engine. You describe a decision — a job offer, a city move, dropping out — and the engine generates three divergent 10-year futures grounded in real economic data, causal rules, and structured AI analysis.

**Live demo:** _coming soon_

---

- [Product spec](SPEC_Product.md) — what it does, feature phases, what's planned
- [Technical spec](SPEC_Technical.md) — data models, agent logic, prompt templates, APIs
- [User guide](USER_Guide.md) — how to run a simulation and read the results
